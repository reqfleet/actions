...
async function execCommand(command: string, options?: child_process.ExecOptions): Promise<string> {
...
function parseStatusThreshold(input: string): { [statusCode: string]: number } {
  const thresholds: { [statusCode: string]: number } = {};
  input.split(',').forEach(part => {
